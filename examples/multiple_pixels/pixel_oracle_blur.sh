...
{
  return (pixel00 + pixel11 + pixel10 + pixel01)/4;
}
...
int main()
{
  ...
  for(int i = 0; i < x; i++)
    for (int j=0; j < y; j++)
      data_target[i+x*j] = target(data_src[i+j*x], data_src[(i+1)+(j+1)*x], data_src[(i)+(j+1)*x], data_src[(i+1)+(j)*x]);


  // transform using synthesized function
  for(int i = 0; i < x; i++)
    for (int j=0; j < y; j++)
      data_tweak[i+x*j] = tweak(data_src[i+j*x], data_src[(i+1)+(j+1)*x], data_src[(i)+(j+1)*x], data_src[(i+1)+(j)*x]);
  ...
  for(int i = 0; i < x; i++)
    for (int j=0; j < y; j++)
      ...
      // they differ; report the expected mapping
        if(count%50==0)
          printf("false (_ bv%d 8)(_ bv%d 8)(_ bv%d 8)(_ bv%d 8) (_ bv%d 8)\n", data_src[i+j*x], data_src[(i+1)+(j+1)*x], data_src[(i)+(j+1)*x], data_src[(i+1)+(j)*x], data_target[i+x*j]);
        count++;
      }


  // they matchindexx*indexy
  printf("true (_ bv%d 8) (_ bv%d 8) (_ bv%d 8) (_ bv%d 8)(_ bv%d 8)\n", 0,0,0,0, 0);
}
EOM
