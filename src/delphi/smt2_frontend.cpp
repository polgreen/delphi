...
int negation_solver(std::istream &in, decision_proceduret &solver, decision_proceduret &solver2, message_handlert &message_handler)
{
  messaget message(message_handler);

  negation_oracle_solvert oracle_solver(solver, solver2, message_handler);
...
int solver(std::istream &in, decision_proceduret &solver, message_handlert &message_handler)
{
  messaget message(message_handler);

  oracle_solvert oracle_solver(solver, message_handler);
  smt2_solvert smt2_solver{in, oracle_solver};
...
// warning simplify won't work with oracles until freezing is implemented.
  if (cmdline.isset("negation-solver"))
  {
    if (cmdline.isset("bitblast") && cmdline.isset("simplify"))
    {
      ...
      return negation_solver(in, subsolver, subsolver2, message_handler);
    }
    ...
    else
    {
      ...
      smt2_dect negation_subsolver(
          ...
          logic, smt2_dect::solvert::CVC4, message_handler);
      return negation_solver(in, subsolver, negation_subsolver, message_handler);
    }
  }
  else
  {
